...
class CodexWorker(BaseWorker):
    ...
    def _run(self, task: WorkerTaskInput) -> dict[str, str]:
        ...
        cmd, use_stdin_prompt = build_cli_command(
            ...
            default_template=CODEX_CLI_CMD,
        )
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                raise RuntimeError(f"Codex CLI not found: {CODEX_CLI_BIN}") from exc
        ...
        if completed is None:
            raise RuntimeError("Codex execution failed before process start")
